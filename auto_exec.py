...
def _ensure_brackets(client, account_id_key: str, symbol: str, lifecycle, log_prefix: str = '') -> None:
    ...
    try:
        ...
        if not lifecycle.stop_order_id and lifecycle.stop is not None:
            stop_oid = client.place_equity_stop_order(
        ...
        if not lifecycle.tp0_order_id and lifecycle.tp0 is not None:
            tp_oid = client.place_equity_limit_order(
                account_id_key=account_id_key,
                symbol=symbol,
                action='SELL',
                qty=int(lifecycle.filled_qty),
        ...
        for idx, raw in enumerate(list(lst)):
            ...
            try:
                ...
                oid = client.place_equity_limit_order(
                    ...
                    client_order_id=_mk_client_order_id(lifecycle.lifecycle_id, 'EN'),
                )
                ...
                lifecycle.last_entry_eval = f"SENT: entry_limit oid={oid}"
...
def _reconcile_one(client: ETradeClient, account_id_key: str, state: Dict[str, Any], lifecycle: TradeLifecycle, cfg: AutoExecConfig, fetch_last_price_fn=None) -> None:
    ...
    def _order_is_inactive(order_id: int) -> bool:
        """Conservative: treat UNKNOWN as inactive because get_order_status_and_filled_qty searches OPEN + CANCEL_REQUESTED."""
        try:
    ...
    if lifecycle.stage == "CANCEL_PENDING":
        ...
        _persist_lifecycle(state, lifecycle.symbol, lifecycle)
        return
    ...
    if lifecycle.stage == "IN_POSITION" and int(lifecycle.filled_qty or 0) > 0:
        ...
        try:
            if not lifecycle.stop_order_id and lifecycle.stop is not None:
                ...
                lifecycle.notes = (lifecycle.notes or "") + " | stop_recreated"
            if not lifecycle.tp0_order_id and lifecycle.tp0 is not None:
                ...
                lifecycle.notes = (lifecycle.notes or "") + " | tp0_recreated"
        except Exception as e:
        ...
        if lifecycle.stop_order_id:
            ...
            if soid is not None:
                ...
                if (
                    ...
                    if rem > 0:
                        try:
                            mid = client.place_equity_market_order(
                                account_id_key=account_id_key,
                                symbol=lifecycle.symbol,
                                qty=rem,
                                action="SELL",
                                market_session="REGULAR",
                                client_order_id=_mk_client_order_id(lifecycle.lifecycle_id, 'MK'),
                            )
    ...
    for sym, qty in (positions or {}).items():
        ...
        try:
            client.place_equity_market_order(
                ...
                action="SELL",
                market_session="REGULAR",
                client_order_id=_mk_client_order_id(f'FLAT{sym}{now.strftime("%H%M")}', 'MK'),
            )
        except Exception:
            pass
