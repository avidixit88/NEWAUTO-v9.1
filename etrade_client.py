...
class ETradeClient:
    ...
    def get_request_token(self, callback_url: str = "oob") -> OAuthTokens:
        ...
        # requests-oauthlib's fetch_request_token may use POST depending on version,
        # which can lead to non-obvious failures. We therefore perform the signed
        # GET explicitly and parse the URL-encoded response.
        r = tmp.get(url, timeout=20)
        if not r.ok:
            raise RuntimeError(f"Token request failed with code {r.status_code}, response was {r.text!r}.")
        qs = parse_qs(r.text)
        ot = (qs.get("oauth_token") or [None])[0]
        ots = (qs.get("oauth_token_secret") or [None])[0]
        if not ot or not ots:
            raise RuntimeError(f"Token request response missing oauth_token fields: {r.text!r}")
    ...
    def get_access_token(self, request_token: str, request_token_secret: str, verifier: str) -> OAuthTokens:
        ...
        ots = (qs.get("oauth_token_secret") or [None])[0]
        if not ot or not ots:
            raise RuntimeError(f"Access token response missing oauth_token fields: {r.text!r}")
        return OAuthTokens(str(ot), str(ots))
    ...
    def preview_order(self, account_id_key: str, order: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # E*TRADE docs show Order as a LIST (even for a single order).
        payload = {
            "PreviewOrderRequest": {
                "orderType": "EQ",
                "Order": [order],
